<template>
  <div v-if="bounds" id="selection-map">
    <!-- class="m-b-lg" -->
    <div v-if="safeOptions" id="safetoswim-legend">
      <div class="div-icon unsafe" />
      Recently Unsafe <br />
      Last measured safe: <br />
      <div class="div-icon under7" />
      within 7 days <br />
      <div class="div-icon under14" />
      within 14 days <br />
      <div class="div-icon under30" />
      within 30 days <br />
      <div class="div-icon under365" />
      within 1 year <br />
      <div class="div-icon over365" />
      over 1 year ago<br />
    </div>
    <l-map :bounds="bounds" class="map-height" :options="options">
      ...
      >
        <l-popup class="popup">
          ...
          <p v-if="marker.value.LocalWaterbody">
            on {{ marker.value.LocalWaterbody }}
          </p>
          <div v-if="safeOptions">
            ...
            <p>
              Latest: {{ format(Number(marker.value.latest.date), "M/D/YYYY") }}
            </p>
          ...
          <sui-button
            ...
            basic
            fluid
            @click="safetoswimChart(marker.value)"
            >Historical Chart</sui-button
          >
          ...
          <sui-button
            ...
            basic
            fluid
            @click="stationInfo(marker.value)"
            >Station Info</sui-button
          >

          <div v-if="!safeOptions">
            <div v-if="isLoaded(marker)">
              <sui-button color="red" fluid @click="removeStation(marker.value)"
                >Remove Station</sui-button
              >
            </div>
            <div v-else>
              <sui-button color="blue" fluid @click="addStation(marker.value)"
                >Add Station</sui-button
              >
            </div>
          </div>
        </l-popup>
...
import { getParamInfoLine } from "../utils/charts";
  ...
  computed: {
    ...
    chartOptions: function () {
      ...
      const options = {
        ...
        exporting: {
          filename: `line chart ${this.selection.activeParam} ${this.getYearRange}`,
        },
      };
      ...
      if (index !== -1) {
        ...
        if (safeValue) {
          ...
          if (
            ...
            if (dt > days7) safety = "under7";
            else if (dt > days14) safety = "under14";
            else if (dt > days30) safety = "under30";
            else if (dt > days365) safety = "under365";
          }
